...
// Overrideables - add a toolbar next to the tab control
	bool CreateTabControl()
	{
		...
		if(bRet)
		{
			...
			m_wndTB = AtlCreateSimpleToolBar(m_hWnd, NULL, FALSE, dwStyle);

			{
				TBBUTTON tbb[3] = {0};

				tbb[0].idCommand = ID_WINDOW_SHOW_VIEWS;
				tbb[0].fsState = TBSTATE_ENABLED;
				tbb[0].fsStyle = BTNS_BUTTON;
				tbb[0].iBitmap = -1; // Index of the bitmap

				tbb[1].idCommand = ID_WINDOW_CLOSE;
				tbb[1].fsState = TBSTATE_ENABLED;
				tbb[1].fsStyle = BTNS_BUTTON;
				tbb[1].iBitmap = -1; // Index of the bitmap
			}
	...
	void UpdateTooltipText(LPNMTTDISPINFO pTTDI)
	{
		...
		m_strTooltip = GetPageTitle( static_cast<int>(pTTDI->hdr.idFrom));

		CAxWindow wnd = GetPageHWND(static_cast<int>(pTTDI->hdr.idFrom));
